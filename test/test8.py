            ...
            if tecnico in vinculo_tecnico_auxiliares:
                ...
                for auxiliar in vinculo_tecnico_auxiliares[tecnico]:
                    ...
                    if auxiliar == tecnico and (tecnico, tecnico) in contagem_por_auxiliar:
                        ...
                        total[0] += aux_total
                        categorias = ", ".join(contagem_por_auxiliar[(tecnico, tecnico)].keys())
                        arquivo.write(f"    Serviço: {tecnico}, Categorias: {categorias}, Quantidade: {aux_total}\n")
                    elif (tecnico, auxiliar) in contagem_por_auxiliar:
                        aux_total = sum(contagem_por_auxiliar[(tecnico, auxiliar)].values())
                        total[0] += aux_total
                        categorias = ", ".join(contagem_por_auxiliar[(tecnico, auxiliar)].keys())
                        arquivo.write(f"    Serviço: {auxiliar}, Categorias: {categorias}, Quantidade: {aux_total}\n")
