...
def processar_planilha(caminho, tecnicos_a_evitar, auxiliares_a_evitar, caminho_arquivo_saida):
    ...
    for (tecnico, auxiliar), contagem_por_categoria in contagem_por_auxiliar.items():
        saida += f"Técnico: {tecnico}, Auxiliar: {auxiliar}\n"
        for categoria, quantidade in contagem_por_categoria.items():
            saida += f"  Categoria: {categoria}, Quantidade de Ajuda: {quantidade}\n"

        saida += f"  Total de Serviços para {tecnico}: {contagem_total_por_tecnico[tecnico]}\n\n"
...
caminho_arquivo_saida = "saida.txt"
